...
function mapProductToPackage(product: StoreProduct) {
  const fallbackColors: Record<string, { color: string }> = {
    basic: { color: "from-blue-600 to-cyan-500" },
    pro: { color: "from-purple-600 to-violet-500" },
    premium: { color: "from-purple-500 to-pink-500" },
  };

  const style = fallbackColors[product.slug] ?? { color: "from-primary to-secondary" };
...
export default function PackageDetail() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // Keep fallback package content if network fails.
      } finally {
  ...
  const fallbackPkg = id ? packagesData[id as PackageId] : undefined;
  const pkg = storeProduct ? mapProductToPackage(storeProduct) : fallbackPkg;
  ...
  const description = `هذه الباقة تقدم ${pkg.features.join("، ")} وتمنحك مسارًا احترافيًا من التصميم إلى التنفيذ. هذه الباقة تدعم مشروعك خطوة بخطوة، وتوفر كل عنصر يحتاجه الموقع بشكل احترافي، من الصفحة الأولى إلى التسليم النهائي، مع متابعة متواصلة ودعم مرن لتحقيق نتائج مضمونة. تجربة المستخدم هنا مصممة لتكون سريعة، واضحة، وقابلة للتطوير حسب احتياجاتك المتغيرة.`;
  ...
  const handleBuyNow = () => {
    ...
    toast({ title: "تمت إضافة الباقة للسلة", description: pkg.name });
    navigate("/cart");
  };
            ...
            <div className="p-8 flex items-center justify-center">
              <div className="relative w-full max-w-[36rem] aspect-square flex items-center justify-center rounded-[2rem] border border-white/10 bg-gradient-to-br from-slate-900 to-slate-800 overflow-hidden">
                {storeProduct?.image_url ? (
                  <img src={storeProduct.image_url} alt={pkg.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="text-center">
                    <LayoutDashboard className="mx-auto mb-4 h-24 w-24 text-secondary" />
                    <p className="text-sm text-white/60">صورة الباقة</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="px-6 py-8 sm:px-10 sm:py-10 flex flex-col justify-center items-center text-center">
              <div className="space-y-6 w-full max-w-3xl mx-auto">
                <div className="flex flex-col gap-2 items-center text-center">
                  ...
                  <p className="text-white/60 text-sm">{pkg.subtitle}</p>
                </div>

                <div className="inline-flex items-baseline justify-center gap-2 rounded-3xl bg-white/5 px-5 py-3 mx-auto">
                  <span className="text-3xl font-black text-secondary">{pkg.priceSar}</span>
                ...
                <Button
                  ...
                  className="mx-auto w-full max-w-xs rounded-3xl bg-gradient-to-r from-primary to-secondary py-4 text-base font-bold text-white shadow-xl shadow-primary/20"
                >
                  شراء الآن
                </Button>
              </div>
